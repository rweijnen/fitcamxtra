...
These five ship with a label and a control, and the command number is confirmed
from the dispatch table, but nothing has verified what the parameter means. The
...
| Driving Collision Sensing | 2011 | 0 | That 2011 is the G-sensor. The table calls it `Config_Snapshot_SensorLevel`, which may be a stills setting | Set it high, knock the car, see whether a clip is locked. If nothing changes, it is not the G-sensor |
...
| Parking Collision Detection | 3038 | 2 | That 3038 is parking sensitivity. 3038, 8005 and 8020 all report 2 and the table maps all three to config index 0x5b, which cannot be true | Change parking sensitivity on the camera, re-read all three, see which moves |
...
| 3002 | Supported commands | Would answer the detection question above, and several others. Never tried |
...
| 3009 | Display mode | Unknown what it switches |
